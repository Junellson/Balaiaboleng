...
$tahunanexport = $_POST['tahunanexport'];
$cariperiode = $tahunanexport."-";
$headerperiode = "Periode Tahun " . $tahunanexport;
...
header('Content-Disposition: attachment; filename="Laporan Tahun '.$tahunanexport.'.xlsx"'); // Set nama file excel nya
$spreadsheet->setActiveSheetIndex(0);
$activeWorksheet->setTitle("TAHUNAN");
...
$activeWorksheet->setCellValue('A1', 'Export Laporan Tahunan');
...
$first_grafik_row = 6;
$q_loopmenugrafik = mysqli_query($koneksi, "SELECT *, tb_menu.id_menu as id_menunya FROM tb_cart JOIN tb_menu ON (tb_cart.id_menu=tb_menu.id_menu) WHERE tb_cart.tgl_transaksi like '%$cariperiode%' GROUP BY tb_menu.id_menu");
while ($loopmenugrafik = mysqli_fetch_array($q_loopmenugrafik)) {
    ...
    $q_cariqty = mysqli_query($koneksi, "SELECT SUM(qty) as total_qty FROM tb_cart WHERE tgl_transaksi like '%$cariperiode%' AND id_menu='$loopmenugrafik[id_menunya]'");
...
$dsl=array(
    new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, 'TAHUNAN!$I$5', NULL, 1, [], NULL, "6488ea"), //Cause of the error
);



$xal=array(
    new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, 'TAHUNAN!$H$6:$H$'.$first_grafik_row, NULL, $first_grafik_row),
);

$dsv = array(
    new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_NUMBER, 'TAHUNAN!$I$6:$I$'.$first_grafik_row, NULL, $first_grafik_row),
);
